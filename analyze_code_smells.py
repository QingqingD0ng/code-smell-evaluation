...
from generate_code import PRODUCTION_MODELS, DEBUG_MODELS, DATASET_NAME, PROMPT_TEMPLATES
import argparse
...
def capture_analysis_output(func, *args, **kwargs):
    """Capture the output of an analysis function"""
    import io
    from contextlib import redirect_stdout
...
def save_analysis_to_file(model_name, analysis_type, content):
    """Save analysis content to a file"""
    filename = f"analysis_{model_name}_{analysis_type}.txt"
    with open(filename, 'w', encoding='utf-8') as f:
        f.write(content)
    return filename
...
def analyze_with_pylint(file_path):
    ...
    try:
        ...
        try:
            ...
            fatal_errors = [item for item in output if item.get("message-id", "").startswith('F')]
...
def is_basic_smell(item):
    ...
    FILTERS = ['C0303', 'C0304', 'C0305', 'C0103', 'C0112', 'C0114', 'C0115', 'C0116', 'W0611', 'C0415', 'W0404',
           'C0413', 'C0411', 'C0412', 'W0401', 'W0614', 'C0410', 'C0413', 'C0414', 'R0402', 'C2403', 'E0401']

    msg_id = item.get("message-id", "")
    # Check if the message ID is in our filters or starts with F (fatal errors)
...
def analyze_folder(folder_path, output_dir):
    ...
    technique_output_dir = os.path.join(output_dir, technique_name)
    ...
    logging.info(f"Analyzing {len(py_files)} files in {technique_name}")
    ...
    try:
        with open(pylint_csv, 'w', newline='', encoding='UTF8') as f:
            writer = csv.writer(f)
            writer.writerow(['Task ID', 'Message ID', 'Symbol', 'Message', 'Line', 'Type'])
        ...
        for task_id, issues in result['pylint'].items():
            if issues['fatal_errors']:
                ...
                for error in issues['fatal_errors']:
                    print(f"  - Line {error.get('line')}: [{error.get('message-id')}] {error.get('message')}")
...
def analyze_top_smells(results):
    ...
    for rank, (smell, count) in enumerate(top_smells, 1):
        ...
        for example in smell_details[smell]:
            print(f"  - {example['technique']}/{example['task']} (line {example['line']}): {example['message']}")
...
def analyze_top_smells_by_type(results):
    ...
    for msg_type, type_name in smell_types.items():
        ...
        top_smells = sorted(type_counts[msg_type].items(), 
                          key=lambda x: x[1], 
                          reverse=True)[:5]
        ...
        for rank, (smell, count) in enumerate(top_smells, 1):
            ...
            for example in type_details[msg_type][smell]:
                print(f"  - {example['technique']}/{example['task']} "
...
def generate_statistics_table(results):
    ...
    for technique, result in results.items():
        ...
        total_instances = error_count + convention_count + refactor_count + warning_count
    ...
    print("|" + "|".join(f" {v:^20} " for v in total_row))
...
def generate_security_statistics_table(results):
    ...
    print("\nTop Security Smells:")
    print("|".join(f" {h:^20} " for h in ["Security Smell ID", "Count"]))
    print("|" + "|".join("-" * 22 for _ in range(2)) + "|")
    
    for smell_id, count in sorted(security_types.items(), key=lambda x: x[1], reverse=True):
        print("|" + "|".join(f" {v:^20} " for v in [smell_id, str(count)]))
...
def generate_top_security_smells_table(results):
    ...
    for technique, result in results.items():
        ...
        if technique != list(results.keys())[-1]:  # Don't print separator after last dataset
            print("|" + "|".join("-" * 32 for _ in headers) + "|")
    ...
    for result in results.values():
        if result['bandit'] and 'results' in result['bandit']:
            ...
            for item in result['bandit']['results']:
                ...
                all_security_smells[test_id]['cwe'] = item.get('issue_cwe', {}).get('id', 'N/A')
    
    # Get top 3 overall security smells
    top_overall = sorted(
        all_security_smells.items(),
        key=lambda x: x[1]['count'],
        reverse=True
    )[:3]
...
def generate_comprehensive_table(results):
    ...
    print("|" + "|".join("-" * 17 for _ in headers) + "|")
    ...
    print("|" + "|".join(f" {v:^15} " for v in total_row))

def main():
    # Add argument parser for debug mode
    parser = argparse.ArgumentParser(description='Analyze code smells')
    parser.add_argument('--debug', action='store_true', help='Run in debug mode with smaller models')
    ...
    for model_name, _ in DEBUG_MODELS if args.debug else PRODUCTION_MODELS:
        model_results = {}
        ...
        # Analyze each technique for this model
        for technique in TECHNIQUES:
            technique_dir = os.path.join(model_code_dir, technique)
            if not os.path.exists(technique_dir):
                logging.warning(f"No code directory found for {technique_dir}")
                continue
                
            pylint_results, bandit_results = analyze_folder(technique_dir, model_dir)
            if pylint_results is not None:
                model_results[technique] = {
    ...
    try:
        with open(summary_csv, 'w', newline='', encoding='UTF8') as f:
            writer = csv.writer(f)
            writer.writerow(['Model', 'Technique', 'Total Files', 'Total Pylint Issues', 'Total Bandit Issues'])
            
            for model_name, model_results in results.items():
                for technique, result in model_results.items():
                    total_files = sum(1 for _ in result['pylint'].values())
                    total_pylint = sum(len(issues['fatal_errors']) + len(issues['other_issues']) 
                                     for issues in result['pylint'].values())
                    total_bandit = len(result['bandit'].get('results', [])) if result['bandit'] else 0
                    
                    writer.writerow([model_name, technique, total_files, total_pylint, total_bandit])
        ...
        for model_name, model_results in results.items():
            ...
            analyses = {
                ...
                'comprehensive': generate_comprehensive_table
            }
            ...
            for analysis_name, analysis_func in analyses.items():
                ...
                with open(os.path.join(model_dir, f"{analysis_name}.txt"), 'w', encoding='utf-8') as f:
                    f.write(output)
...
- summary.txt
"""
        with open(os.path.join(analysis_dir, "overall_summary.txt"), 'w', encoding='utf-8') as f:
