...
def extract_python_code(text):
    ...
    if matches:
        ...
        if any(line.strip().startswith(('def ', 'class ', 'import ', 'from ', '#', 'if ', 'for ', 'while ')) 
    ...
    python_indicators = ('def ', 'class ', 'import ', 'from ', '#', 'if ', 'for ', 'while ', 
                        'return ', 'print(', 'with ', 'try:', 'except:', 'finally:', '@')
...
def create_model_folders(base_path, models):
    ...
    for model_name, _ in models:
        ...
        # Create technique folders for each model
        for technique in PROMPT_TEMPLATES.keys():
            technique_path = os.path.join(model_path, technique)
            os.makedirs(technique_path, exist_ok=True)

def save_code_to_file(code, model_path, technique, task_id):
    """Save extracted code to a Python file."""
    if not code.strip():
        return False
    
    filename = f"{task_id}.py"
    file_path = os.path.join(model_path, technique, filename)
            ...
            for model_name, _ in models:
                ...
                for technique in PROMPT_TEMPLATES.keys():
                    ...
                    if technique in ['cot', 'rci']:
                        column_name = f"{model_name}_{technique}_final" if technique == 'cot' else f"{model_name}_{technique}_improved"
                    else:
                    ...
                    if column_name in row:
                        ...
                        if save_code_to_file(code, model_path, technique, task_id):
...
def main():
    ...
    # Configure paths
    csv_path = "debug_results.csv" if args.debug else "bigcodebench_prompt_comparison.csv"
